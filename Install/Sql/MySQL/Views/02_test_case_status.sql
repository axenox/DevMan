...
SELECT
	tmp.*,
	CASE
		WHEN tmp.last_test_time IS NOT NULL AND (tmp.last_change_time IS NULL OR tmp.last_change_time < tlc2.tested_build_time) AND tlc2.test_ok_flag = 1 THEN 'PASS'
		WHEN tmp.last_test_time IS NOT NULL AND (tmp.last_change_time IS NULL OR tmp.last_change_time < tlc2.tested_build_time) AND tlc2.test_ok_flag = 0 THEN 'FAIL'
		ELSE 'TODO'
	END AS test_status,
